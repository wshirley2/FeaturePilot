...
def _parse_args():
    p = argparse.ArgumentParser(
        prog="featurepilot-engine",
        description="FeaturePilot internal runtime REPL. Works with any OpenAI-compatible LLM.",
    )
    p.add_argument("-m", "--model", help="Model name (default: $FEATUREPILOT_MODEL or gpt-5.5)")
...
def main():
    ...
    if not config.api_key:
        ...
        console.print(
            "Set one of: OPENAI_API_KEY, DEEPSEEK_API_KEY, or FEATUREPILOT_API_KEY\n"
            "\nExamples:\n"
            "  # OpenAI\n"
            ...
            "\n"
            "  # Ollama (local)\n"
            "  export OPENAI_API_KEY=ollama OPENAI_BASE_URL=http://localhost:11434/v1 FEATUREPILOT_MODEL=qwen2.5-coder\n"
        )
        sys.exit(1)
...
def _repl(agent: Agent, config: Config):
    ...
    console.print(Panel(
        f"[bold]FeaturePilot[/bold] v{__version__}\n"
    ...
    while True:
        ...
        if user_input == "/save":
            ...
            console.print(f"Resume with: python -m featurepilot.engine -r {sid}")
            continue
        if user_input == "/diff":
...
def _show_help():
    console.print(Panel(
        ...
        "  quit           Exit FeaturePilot\n"
        "\n"
        "[bold]Input:[/bold]\n"
        "  Enter          Submit message\n"
        "  Esc+Enter      Insert newline (for pasting code)",
        title="FeaturePilot Engine Help",
        border_style="dim",
    ))
