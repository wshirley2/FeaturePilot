    ...
    *,
    ...
    def source(*names: str) -> str | None:
        ...
        if user is not None:
            ...
            field = {
                "CORECODER_PROVIDER": "provider",
                "OPENAI_BASE_URL": "base_url",
                "CORECODER_BASE_URL": "base_url",
                "FEATUREPILOT_MODEL": "model",
                "CORECODER_MODEL": "model",
                "CORECODER_API_KEY": "api_key",
    ...
    resolved_model = model or source("FEATUREPILOT_MODEL", "CORECODER_MODEL") or "gpt-5.5"
    resolved_api_key = api_key or source("CORECODER_API_KEY", "OPENAI_API_KEY", "DEEPSEEK_API_KEY") or ""
    resolved_base_url = base_url or source("OPENAI_BASE_URL", "CORECODER_BASE_URL")
    provider = source("CORECODER_PROVIDER") or "openai"
    max_context = os.getenv("CORECODER_MAX_CONTEXT") or project.get("CORECODER_MAX_CONTEXT")
    return Config(
        model=resolved_model,
        api_key=resolved_api_key,
        base_url=resolved_base_url,
        max_tokens=int(os.getenv("CORECODER_MAX_TOKENS") or project.get("CORECODER_MAX_TOKENS") or "4096"),
        temperature=float(os.getenv("CORECODER_TEMPERATURE") or project.get("CORECODER_TEMPERATURE") or "0"),
...
def _project_dotenv_values() -> dict[str, str]:
    if os.getenv("CORECODER_LOAD_DOTENV", "1").lower() in {"0", "false", "no", "off"}:
        return {}
    try:
