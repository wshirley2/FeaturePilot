...
class Config:
    ...
    def from_env(cls) -> "Config":
        ...
        load_dotenv = os.getenv("TECHPILOT_LOAD_DOTENV", "1").lower()
        if load_dotenv not in {"0", "false", "no", "off"}:
            _load_dotenv()
        # pick up common env vars automatically
        api_key = (
            os.getenv("TECHPILOT_API_KEY")
            or os.getenv("OPENAI_API_KEY")
            or os.getenv("DEEPSEEK_API_KEY")
            or ""
        )
        model = os.getenv("TECHPILOT_MODEL", "gpt-5.5")
        configured_context = os.getenv("TECHPILOT_MAX_CONTEXT")
        return cls(
            model=model,
            api_key=api_key,
            base_url=os.getenv("OPENAI_BASE_URL") or os.getenv("TECHPILOT_BASE_URL"),
            max_tokens=int(os.getenv("TECHPILOT_MAX_TOKENS", "4096")),
            temperature=float(os.getenv("TECHPILOT_TEMPERATURE", "0")),
            max_context_tokens=(
                int(configured_context)
                if configured_context is not None
                else default_context_tokens_for_model(model)
            ),
            provider=os.getenv("TECHPILOT_PROVIDER", "openai"),
        )
