...
def _parse_args():
    p = argparse.ArgumentParser(
        prog="techpilot-engine",
        description="TechPilot internal runtime REPL. Works with any OpenAI-compatible LLM.",
    )
    p.add_argument("-m", "--model", help="Model name (default: $TECHPILOT_MODEL or gpt-5.5)")
...
def main():
    ...
    if not config.api_key:
        ...
        console.print(
            "Set one of: OPENAI_API_KEY, DEEPSEEK_API_KEY, or TECHPILOT_API_KEY\n"
            "\nExamples:\n"
            "  # OpenAI\n"
            ...
            "\n"
            "  # Ollama (local)\n"
            "  export OPENAI_API_KEY=ollama OPENAI_BASE_URL=http://localhost:11434/v1 TECHPILOT_MODEL=qwen2.5-coder\n"
        )
        sys.exit(1)
...
def _repl(agent: Agent, config: Config):
    ...
    console.print(Panel(
        f"[bold]TechPilot[/bold] v{__version__}\n"
    ...
    hist_path = os.path.expanduser("~/.techpilot_history")
    ...
    while True:
        ...
        if user_input == "/save":
            ...
            console.print(f"Resume with: python -m techpilot.engine -r {sid}")
            continue
        if user_input == "/diff":
