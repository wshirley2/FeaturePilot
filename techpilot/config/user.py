...
def user_config_directory() -> Path:
    ...
    override = os.getenv("TECHPILOT_CONFIG_DIR")
    if override:
        return Path(override).expanduser()
    if sys.platform == "win32":
        appdata = os.getenv("APPDATA")
        if appdata:
            return Path(appdata) / "TechPilot"
        return Path.home() / "AppData" / "Roaming" / "TechPilot"
    return Path.home() / ".config" / "techpilot"
    ...
    *,
    ...
    def source(*names: str) -> str | None:
        ...
        if user is not None:
            ...
            field = {
                "TECHPILOT_PROVIDER": "provider",
                "OPENAI_BASE_URL": "base_url",
                "TECHPILOT_BASE_URL": "base_url",
                "TECHPILOT_MODEL": "model",
                "TECHPILOT_API_KEY": "api_key",
    ...
    resolved_model = model or source("TECHPILOT_MODEL") or "gpt-5.5"
    resolved_api_key = api_key or source("TECHPILOT_API_KEY", "OPENAI_API_KEY", "DEEPSEEK_API_KEY") or ""
    resolved_base_url = base_url or source("OPENAI_BASE_URL", "TECHPILOT_BASE_URL")
    provider = source("TECHPILOT_PROVIDER") or "openai"
    max_context = os.getenv("TECHPILOT_MAX_CONTEXT") or project.get("TECHPILOT_MAX_CONTEXT")
    return Config(
        model=resolved_model,
        api_key=resolved_api_key,
        base_url=resolved_base_url,
        max_tokens=int(os.getenv("TECHPILOT_MAX_TOKENS") or project.get("TECHPILOT_MAX_TOKENS") or "4096"),
        temperature=float(os.getenv("TECHPILOT_TEMPERATURE") or project.get("TECHPILOT_TEMPERATURE") or "0"),
...
def _project_dotenv_values() -> dict[str, str]:
    if os.getenv("TECHPILOT_LOAD_DOTENV", "1").lower() in {"0", "false", "no", "off"}:
        return {}
    try:
