"""Bridge TechPilot's Tool protocol to its Plan-aware policy."""
...
_TECHPILOT_TOOL_NAMES = ("read_file", "glob", "grep", "edit_file", "write_file", "bash", "now")


def build_techpilot_tools() -> list[Tool]:
    """Return the deliberately small Tool set allowed for a TechPilot Run."""

    tools: list[Tool] = []
    for name in _TECHPILOT_TOOL_NAMES:
        tool = get_tool(name)
        if tool is None:
            raise RuntimeError(f"TechPilot tool registry is missing required tool {name!r}")
        tools.append(tool)
    return tools
