...
class ToolPolicy:
    """Apply TechPilot's first-version workspace and Plan restrictions."""
    ...
    def decide(self, tool_name: str, arguments: dict[str, Any], context: ExecutionContext) -> PolicyDecision:
        ...
        if tool_name in {"fetch_url", "agent"}:
            return self._deny(effect, f"{tool_name} is disabled for TechPilot runs")
        if tool_name == "now":
        ...
        return self._deny(effect, f"Tool {tool_name!r} is not available in a TechPilot run")
