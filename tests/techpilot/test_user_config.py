...
from techpilot.config.user import (
    UserConfig,
    load_user_config,
...
def test_first_run_enters_setup_wizard_and_then_uses_tui(tmp_path, monkeypatch):
    monkeypatch.setenv("TECHPILOT_CONFIG_DIR", str(tmp_path / "user-config"))
    calls = []
    ...
    monkeypatch.setattr("techpilot.cli.run_setup_wizard", lambda repairing: calls.append(("wizard", repairing)))
    monkeypatch.setattr("techpilot.cli.TechPilotTui", FakeTui)
    monkeypatch.setattr("techpilot.cli.RuntimeBootstrap", Bootstrap)
    monkeypatch.setattr("techpilot.cli.tui_supported", lambda: True)
    monkeypatch.setattr("techpilot.cli.sys.stdin", type("Input", (), {"isatty": lambda self: False})())
...
def test_complete_user_config_does_not_repeat_setup(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("techpilot.cli.run_setup_wizard", lambda repairing: calls.append(("wizard", repairing)))
    monkeypatch.setattr("techpilot.cli.TechPilotTui", FakeTui)
    monkeypatch.setattr("techpilot.cli.RuntimeBootstrap", lambda: type("Bootstrap", (), {"build": lambda self, inputs: object()})())
    monkeypatch.setattr("techpilot.cli.tui_supported", lambda: True)
    monkeypatch.setattr("techpilot.cli.sys.stdin", type("Input", (), {"isatty": lambda self: False})())
...
def test_damaged_user_config_enters_repair_wizard(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("TECHPILOT_CONFIG_DIR", str(directory))
    calls = []
    ...
    monkeypatch.setattr("techpilot.cli.run_setup_wizard", lambda repairing: calls.append(repairing))
    monkeypatch.setattr("techpilot.cli.TechPilotTui", FakeTui)
    monkeypatch.setattr("techpilot.cli.RuntimeBootstrap", lambda: type("Bootstrap", (), {"build": lambda self, inputs: object()})())
    monkeypatch.setattr("techpilot.cli.tui_supported", lambda: True)
    monkeypatch.setattr("techpilot.cli.sys.stdin", type("Input", (), {"isatty": lambda self: False})())
...
def test_default_entry_falls_back_to_text_cli_without_a_tty(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("techpilot.cli.ChatSession", FakeChatSession)
    monkeypatch.setattr("techpilot.cli.RuntimeBootstrap", lambda: type("Bootstrap", (), {"build": lambda self, inputs: object()})())
    monkeypatch.setattr("techpilot.cli.tui_supported", lambda: False)
...
def test_runtime_configuration_priority_is_cli_then_environment_then_user_then_dotenv(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("TECHPILOT_MODEL", "environment-model")
...
def test_invalid_user_configuration_is_not_treated_as_complete(tmp_path, monkeypatch):
    monkeypatch.setenv("TECHPILOT_CONFIG_DIR", str(tmp_path))
...
def test_setup_wizard_saves_all_fields_without_echoing_api_key(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("techpilot.config.user.getpass", lambda _prompt: "hidden-api-key")
